...
private:
    ...
    private:
        ...
        public:
            ...
            bool MoveNext() override {
                ++offset_;
                return offset_ < segment_.GetSize();
            }

            const T& GetCurrent() const override {
                if (offset_ < 0 || offset_ >= segment_.GetSize())
    ...
    public:
        explicit Segment(int capacity)
            : data_(capacity), head_(0), tail_(0), isEmpty_(true) {
        }

        int GetHead() const { return head_; }
        int GetTail() const { return tail_; }
        int GetSize() const { return isEmpty_ ? 0 : (tail_ - head_ + 1); }
